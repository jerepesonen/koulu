...
fullfile = (str(client.recv(409600000), "utf-8"))
filename = fullfile.split("\n")[1]
header = int(fullfile.split("\n")[0])
fullmsg = fullfile.split("\n")[2]
received = len(fullmsg)
"""while received < header:
	fullmsg = fullmsg+str(client.recv(4096), "utf-8")
	received = received+len(fullfile.split("\n")[2])
"""

fullmsgbytearray = bytearray(fullmsg, "utf-8")

with open(os.path.join(args.path,filename), "wb") as file:
	file.write(fullmsgbytearray)
"""with open(os.path.join(args.path,filename), "wb") as file:
	file.write(str(fullfile.split("\n")[2]))
	header = fullfile.split("\n")[0]
	fullmsg = fullfile.split("\n")[2]
	received = fullmsg
	#received = len(fullmsg)
	while received < header:
		fullmsg = fullmsg+str(client.recv(4096), "utf-8")
		file.write(fullmsg)
		received = received+fullfile.split("\n")[1]	"""
file.close
client.send(bytes("Kiitos!\n", "utf-8"))